...
/**
 * Basic app to allow the user to see data keys for item sheets
 * @internal
 */
export class PDFItemDataBrowser extends Application {
    ...
    getData(options) {
        ...
        const flatten = (data, current = '', danger = DangerLevel.Safe) => {
            let results = [];
            window['itemData'] = this.item.system;
            ...
            if (typeof data === 'object') {
                if (data instanceof Item) {
                    console.warn('Embedded Item found in data, excluding the embedded Item from the list of fields')
                    return results;
                }
        ...
        data['paths'] = flatten(this.item.system, 'system');
        data['paths'].push({
            key: 'name',
